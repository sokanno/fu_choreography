...
# ドロップダウンメニューの作成
modes = ["流れモード", "天上天下唯筒独尊モード"]
# ドロップダウン作成
def on_mode_select(m):
    select = m.text  # 選ばれたモード名
    for mode, widgets in slider_groups.items():
        # 各ウィジェット（slider, wtext）は tuple で登録している
        for w in widgets:
...
add_mode_slider("流れモード", ui, "Separation",    0,   2,   separationFactor, "{:.2f}",
           lambda v: globals().update(separationFactor=v))
add_mode_slider("流れモード", ui, "Cohesion",      0,   2,   cohesionFactor,   "{:.2f}",
           lambda v: globals().update(cohesionFactor=v))
add_mode_slider("流れモード", ui, "Noise Scale",   .01, .5,  noiseScale,      "{:.2f}",
           lambda v: globals().update(noiseScale=v))
add_mode_slider("流れモード", ui, "Wave Scale",    .01, 1.0, waveScale,       "{:.2f}",
           lambda v: globals().update(waveScale=v))
add_mode_slider("流れモード", ui, "Wave Strength", 0,   1,   waveStrength,    "{:.2f}",
           lambda v: globals().update(waveStrength=v))
add_mode_slider("流れモード", ui, "Noise Speed",   0,   .1,  noiseSpeed,      "{:.3f}",
           lambda v: globals().update(noiseSpeed=v))
ui.append_to_caption("<br><br>")

# # ─── Viewport Control ─────────────────────────────────
# ui.append_to_caption("<b>Viewport Control</b><br><br>")
# add_mode_slider(ui, "Rot Speed",     0,   .02, rotationSpeed,   "{:.3f}",
#            lambda v: globals().update(rotationSpeed=v))
# add_mode_slider(ui, "Radius",        2,   20,  radius,          "{:.1f}",
#            lambda v: globals().update(radius=v))
# add_mode_slider(ui, "Cam Height",    0,   10,  cameraHeight,    "{:.1f}",
...
# 3Dビュー背景の明るさ
add_mode_slider("流れモード", ui, "BG3D Bright", 0, 1, bg3d_brightness, "{:.2f}",
           lambda v: (
...
# 2Dビュー背景の明るさ
add_mode_slider("流れモード", ui, "BG2D Bright", 0, 1, bg2d_brightness, "{:.2f}",
           lambda v: (
...
# LED 波の振幅
add_mode_slider("流れモード", ui, "LED Amplitude", 0, 2, led_amp, "{:.2f}",
...
# ─── HSB Color Control ───────────────────────────────
# 3) UI（add_mode_slider／"流れモード", ui.append_to_caption済みと仮定）
ui.append_to_caption("<b>Wave Hue Control</b><br><br>")
add_mode_slider("流れモード", ui, "Hue Offset", 0.0, 1.0, base_hue, "{:.2f}",
           lambda v: globals().update(base_hue=v))
add_mode_slider("流れモード", ui, "Hue Span",   0.0, 1.0, hue_span, "{:.2f}",
