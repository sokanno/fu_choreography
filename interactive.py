...
# ========================================================
# ─── Audience 数スライダー ─────────────────────────
ui.append_to_caption("<b>観客パラメータ</b><br><br>")
...
class Audience:
    ...
    #     dx = math.cos(theta) * self.speed * dt
    #     dy = math.sin(theta) * self.speed * dt
    #     self.x = min(max(self.x + dx, self.xmin), self.xmax)
    #     self.y = min(max(self.y + dy, self.ymin), self.ymax)

    #     # 3D 体幹更新
    #     self.body.pos = vector(self.x, self.y, 0)
    #     # 3D 頭部更新：同じく radius 分下げる
    #     self.head.pos = vector(self.x, self.y, self.cyl_h + self.radius)
    #     # 2D 更新も同じ
    #     self.dot2d.pos = vector(self.x, self.y, -0.01)
    def update(self, dt):
        # 1) ノイズ時間を進める
        self.noise_t += dt * audience_noise_speed

        # 2) Perlin ノイズから滑らかな方向を得る
        #    pnoise2 は [0,1] が返るので、2π にマッピング
